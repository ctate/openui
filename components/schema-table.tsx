  ...
  const renderProperties = (
    ...
    return Object.entries(properties).map(([key, value]) => (
      <React.Fragment key={key}>
        <tr>
          <td
            className={`py-2 px-4 border-b border-gray-700 ${indent > 0 ? "pl-" + indent * 4 : ""}`}
          >
            {key}
        ...
        {value.properties && renderProperties(value.properties, indent + 1)}
        {value.patternProperties &&
          renderProperties(value.patternProperties, indent + 1)}
      </React.Fragment>
    ));
  };
